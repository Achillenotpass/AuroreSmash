...
public class PlayersCamera : MonoBehaviour, IUpdateUser
{
    ...
    private void Start()
    {
        ...
        m_XLeft = m_GameManager.CameraCenterPosition.x - m_GameManager.CameraSize.x;
        m_XRight = m_GameManager.CameraCenterPosition.x + m_GameManager.CameraSize.x;
        m_YUp = m_GameManager.CameraCenterPosition.y - m_GameManager.CameraSize.y;
        m_YDown = m_GameManager.CameraCenterPosition.y - m_GameManager.CameraSize.y;
    }
    ...
    private Vector3 CheckForNewPosition(float p_DeltaTime)
    {
        ...
        if (!Physics.Raycast(l_Camera.ScreenPointToRay(new Vector3(0, l_Camera.pixelHeight / 2)), 1000.0f, m_LayerMask))
        {
            ...
            {
                l_XOffset = l_HitInfo.point.x - m_XLeft;
            }
        }
        //Raycast haut
        if (!Physics.Raycast(l_Camera.ScreenPointToRay(new Vector3(l_Camera.pixelWidth / 2, 0)), 1000.0f, m_LayerMask))
        {
            ...
            {
                l_YOffset = l_HitInfo.point.y + m_YUp;
            }
        }
        //Raycast bas
        ...
        if (!Physics.Raycast(l_Camera.ScreenPointToRay(new Vector3(l_Camera.pixelWidth - 1, l_Camera.pixelHeight / 2)), 1000.0f, m_LayerMask))
        {
            ...
            {
                l_XOffset = l_HitInfo.point.x + m_XRight;
            }
        }
